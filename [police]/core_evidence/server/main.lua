    ...
    function(id)
        ...
        if xPlayer ~= nil then
            if NetworkGetEntityOwner(entity) ~= src then
                exports.oxmysql:query(
                    "SELECT " ..
                        Config.EvidenceReportInformationFingerprint .. " FROM `players` WHERE citizenid = :owner LIMIT 1",
                    {
...
AddEventHandler(
    ...
    function(coords, interior)
        ...
        exports.oxmysql:query(
            "SELECT " .. Config.EvidenceReportInformationBlood .. " FROM `players` WHERE citizenid = :owner LIMIT 1",
            {
                ["owner"] = xPlayer.PlayerData.citizenid
            },
            function(reportInfo)
                local time = os.time()
                reportInfo[1].cordinates = "X: " .. coords[1] .. " Y: ".. coords[2] .. " Z: " .. coords[3]
...
AddEventHandler(
    ...
    function(coords, bullet, interior)
        ...
        exports.oxmysql:query(
            "SELECT " .. Config.EvidenceReportInformationBullet .. " FROM `players` WHERE citizenid = :owner LIMIT 1",
            {
                ["owner"] = xPlayer.PlayerData.citizenid
            },
            function(reportInfo)
