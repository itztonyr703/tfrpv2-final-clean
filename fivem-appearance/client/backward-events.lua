...
RegisterNetEvent('qb-clothing:client:loadOutfit', function(oData)
    ...
    if data["pants"] ~= nil then
        SetPedComponentVariation(ped, 143, data["pants"].item, data["pants"].texture, 0)
    end
    ...
    if data["t-shirt"] ~= nil then
        SetPedComponentVariation(ped, 14, data["t-shirt"].item, data["t-shirt"].texture, 0)
    end

    -- Vest
    if data["vest"] ~= nil then
        SetPedComponentVariation(ped, -1, data["vest"].item, data["vest"].texture, 0)
    end
    ...
    if data["shoes"] ~= nil then
        SetPedComponentVariation(ped, -1, data["shoes"].item, data["shoes"].texture, 0)
    end

    -- Bag
    if data["bag"] ~= nil then
        SetPedComponentVariation(ped, -1, data["bag"].item, data["bag"].texture, 0)
    end
