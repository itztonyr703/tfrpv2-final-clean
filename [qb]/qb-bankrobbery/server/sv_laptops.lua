...
RegisterNetEvent('qb-bankrobbery:server:BuyLaptop', function(data)
    ...
    if exports['lj-inventory']:GetItemByName(src, item) then
        if (Player.PlayerData.money.crypto - Laptops[index].price) >= 0 then
            -- Remove USB
            exports['lj-inventory']:RemoveItem(src, item, 1, false)
            ...
            local info = {uses = 3}
            exports['lj-inventory']:AddItem(src, laptop, 1, false, info)
...
RegisterNetEvent('qb-bankrobbery:server:BuyGreyUsb', function()
    ...
    if (Player.PlayerData.money.crypto - greyUSBPrice) >= 0 then
        ...
        local info = {uses = 3}
        exports['lj-inventory']:AddItem(src, "usb_grey", 1, false, info)
...
RegisterNetEvent('qb-bankrobbery:server:LaptopDamage', function(laptop)
    ...
    local item = exports['lj-inventory']:GetItemByName(src, laptop)
    if not item then
    ...
    if (Player.PlayerData.items[item.slot].info.uses - 1) <= 0 then
        ...
        exports['lj-inventory']:RemoveItem(src, item.name, 1, item.slot)
        ...
        exports['lj-inventory']:SetInventory(src, Player.PlayerData.items)
    end
end)
