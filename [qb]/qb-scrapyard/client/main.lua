		...
		if closestScrapyard ~= 0 then
									...
									if IsVehicleValid(GetEntityModel(vehicle)) then
										...
										QBCore.Functions.TriggerCallback('qb-scrapyard:checkOwnerVehicle',function(retval)
											if retval then
												...
												QBCore.Functions.Notify("You can't scrap a vehicle that is owned.", "error")
											end
										end,vehiclePlate)
									else
			...
			if #(pos - vector3(Config.Locations[closestScrapyard]["exchange"].x, Config.Locations[closestScrapyard]["exchange"].y, Config.Locations[closestScrapyard]["exchange"].z)) < 1.5 then
				if not IsPedInAnyVehicle(PlayerPedId()) then
					DrawText3Ds(Config.Locations[closestScrapyard]["exchange"].x, Config.Locations[closestScrapyard]["exchange"].y, Config.Locations[closestScrapyard]["exchange"].z, "~g~E~w~ - Break Vehicle Down")
					if IsControlJustReleased(0, 38) then
...
function ScrapVehicle(vehicle)
	isBusy = true
	local scrapTime = math.random(8000, 17000)
    ...
    local menu = {
        {
            ...
            txt = "Exchange parts for materials",
			isMenuHeader = true
        },
        {
        ...
        {
            header = "Exchange tire",
            txt = "Exchange a Tire for materials",
            params = {
                event = "qb-scrapyard:client:exchangeItems",
                args = {
