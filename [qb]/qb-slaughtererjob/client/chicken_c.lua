	...
	Citizen.Wait(0)
		...
		if dist <= 0.5 then
			if IsControlJustPressed(0, Keys['E']) then -- "E"
				QBCore.Functions.Progressbar("chicken_", "Gathering Live Chicken", math.random(15000, 20000), false, true, {
...
Citizen.CreateThread(function()
	...
	Citizen.Wait(0)
		...
		if dist <= 0.5 then
			if IsControlJustPressed(0, Keys['E']) then -- "E"
				QBCore.Functions.Progressbar("chicken_", "Portion the Chicken..", math.random(2000, 5000), false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
...
Citizen.CreateThread(function()
	...
	Citizen.Wait(0)
		...
		if dist <= 2.5 then
			DrawText3D2(pakowanieX2, pakowanieY2, pakowanieZ, "~g~[E]~w~ To pack portioned chicken")
		end

		if dist <= 0.5 then
			if IsControlJustPressed(0, Keys['E']) then -- "E"
				QBCore.Functions.Progressbar("chicken_", "Packing the Chicken..", math.random(2000, 5000), false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
...
function packedchicken()
   ...
   QBCore.Functions.Notify('You packed the chicken')
    else
...
function processchicken()
   ...
   -- DeleteEntity(prQBCore)
    TriggerServerEvent("tostKurczaki:przerob",1)
	QBCore.Functions.Notify('You Slaughtered A Chicken')
    else
...
function SellChicken()
    ...
    ----
    if(count == 0) then
    QBCore.Functions.Progressbar("_chicken", "Selling Chicken Packages..", 6500, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
   ...
   -- DeleteEntity(prQBCore)
    ...
    QBCore.Functions.Notify('You dont have any Chicken Packages!!!', 'error')
