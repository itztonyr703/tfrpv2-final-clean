...
RegisterNetEvent('gksphone:server:musicAll', function(type, musicid, volume, loop)
  if type == "volume" then
...
Config.Core.Commands.Add(Config.TwitterVerifyCommand, 'Blue Tick Approval', {{name = "auth", help = "true/false"}, {name = "AccountName", help = "Twitter Account Name"}}, true, function(source, args)
  local src = source
  if args[1] then
    local label = ""
    local first = true
    ...
    if auth == "true" then
      ...
      MySQL.Async.execute("UPDATE `gksphone_twitter_accounts` SET `averify`= 0 WHERE gksphone_twitter_accounts.username = @username", {
        ['@username'] = tostring(label)
      }, function (result)
        if (result == 1) then
          TriggerClientEvent(Config.CoreNotify, src, "User's blue tick removed")
        else
          TriggerClientEvent(Config.CoreNotify, src, 'You did not write the required information', 'error')
        end
      end)
...
Config.Core.Commands.Add(Config.InstagramVerifyCommand, 'Blue Tick Approval', {{name = "auth", help = "true/false"}, {name = "AccountName", help = "Instagram Account Name"}}, true, function(source, args)
  local src = source
  if args[1] then
    local label = ""
    local first = true
