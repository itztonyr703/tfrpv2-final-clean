...
Config.Core.Commands.Add('billing', 'Bill A Player',
    { { name = 'id', help = 'Player ID' }, { name = 'amount', help = 'Fine Amount' },
        { name = 'label', help = 'Invoice Description' } }, false, function(source, args)
    ...
    if jobinfo then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                ...
                TriggerClientEvent('gksphone:notifi', source,
                    { title = _U('billing_title'), message = _U('bill_cannotbill'),
                        img = '/html/static/img/icons/logo.png' })
            end
        else
            TriggerClientEvent('gksphone:notifi', source,
                { title = _U('billing_title'), message = _U('bill_notplayer'), img = '/html/static/img/icons/logo.png' })
        end
    else
...
AddEventHandler("gksphone:business:delbill", function(id)
    ...
    local gradecheck = false
    for k, l in ipairs(Config.JobGrade) do
        if e.PlayerData.job.name == k and gradejob >= l  then
            gradecheck = true
            break
        end
    end
    if gradecheck then
            ...
            if data[1] then
                ...
                if Player then
                    TriggerClientEvent('gksphone:notifi', Player.PlayerData.source,
                        { title = _U('bussines_title'), message = data[1].label .. _('bill_invoicecancel'),
                            img = '/html/static/img/icons/logo.png' })
                end
        ...
        TriggerClientEvent('gksphone:notifi', src,
            { title = _U('bussines_title'), message = _U('buss_gradenotbill'), img = '/html/static/img/icons/business.png' })
    end
end)
