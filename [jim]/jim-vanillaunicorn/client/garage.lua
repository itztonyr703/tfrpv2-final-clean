		...
		if v.garage then
			...
			Targets["VanGarage: "..k] =
				exports['qb-target']:AddBoxZone("VanGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="VanGarage: "..k, heading = out[4]+180.0, debugPoly=Config.Debug, minZ=(out.z-1.03)-0.1, maxZ=(out.z-1.03)+1.3 },
					{ options = { { event = "jim-vanillaunicorn:client:Garage:Menu", icon = "fas fa-clipboard", label = "Job Vehicles", job = v.job, coords = v.garage.spawn, list = v.garage.list }, },
					distance = 2.0 })
...
RegisterNetEvent('jim-vanillaunicorn:client:Garage:Menu', function(data)
	...
	vehicleMenu[#vehicleMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = "Close", params = { event = "jim-vanillaunicorn:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		...
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = "Remove Vehicle", params = { event = "jim-vanillaunicorn:client:RemSpawn" } }
	else
