		...
		if v.garage then
			...
			Targets["VanGarage: "..k] =
				exports['qb-target']:AddBoxZone("VanGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="VanGarage: "..k, heading = out[4]+180.0, debugPoly=Config.Debug, minZ=(out.z-1.03)-0.1, maxZ=(out.z-1.03)+1.3 },
					{ options = { { event = "jim-beanmachine:client:Garage:Menu", icon = "fas fa-clipboard", label = Loc[Config.Lan].menu["job_vehicles"], job = v.job, coords = v.garage.spawn, list = v.garage.list }, },
					distance = 2.0 })
...
RegisterNetEvent('jim-beanmachine:client:Garage:Menu', function(data)
	...
	vehicleMenu[#vehicleMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "jim-beanmachine:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-clipboard-list", header = Loc[Config.Lan].menu["vehicle_out_of_garage"],
										...
										params = { event = "jim-beanmachine:client:Garage:Blip", }, }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = Loc[Config.Lan].menu["remove_vehicle"], params = { event = "jim-beanmachine:client:RemSpawn" } }
	else
		...
		for _, v in pairs(data.list) do
			...
			vehicleMenu[#vehicleMenu+1] = { header = name, params = { event = "jim-beanmachine:client:SpawnList", args = { spawnName = spawnName, coords = data.coords } } }
		end
	end
    exports['qb-menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent("jim-beanmachine:client:SpawnList", function(data)
	...
	if oldveh ~= 0 then
		...
		triggerNotify(nil, name..Loc[Config.Lan].error["in_the_way"], "error")
	else
...
RegisterNetEvent("jim-beanmachine:client:RemSpawn", function(data)
...
RegisterNetEvent("jim-beanmachine:client:Garage:Blip", function(data)
	triggerNotify(nil, Loc[Config.Lan].success["job_vehicle_map"])
