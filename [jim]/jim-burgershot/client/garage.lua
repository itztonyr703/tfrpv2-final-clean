...
RegisterNetEvent('jim-burgershot:client:Garage:Menu', function(data)
	...
	vehicleMenu[#vehicleMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "jim-burgershot:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-clipboard-list", header = Loc[Config.Lan].menu["out_of_garage"],
										...
										params = { event = "jim-burgershot:client:Garage:Blip", }, }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = Loc[Config.Lan].menu["remove_vehicle"], params = { event = "jim-burgershot:client:RemSpawn" } }
	else
...
RegisterNetEvent("jim-burgershot:client:Garage:Blip", function(data)
