...
CreateThread(function()
    for k, v in pairs(Config.Peds) do
        ...
        drugpeds = CreatePed(4, v.hash, v.x, v.y, v.z-1, 3374176, false, true)
        SetEntityHeading(drugpeds, v.h)
        FreezeEntityPosition(drugpeds, true)
        SetEntityInvincible(drugpeds, true)
        SetBlockingOfNonTemporaryEvents(drugpeds, true)
        TaskStartScenarioInPlace(drugpeds, v.scenario, 0, true)
...
--<!>-- BLIPS START --<!>--
if Config.Blips.UseBlips == true then
	CreateThread(function()
...
if Config.Zones.UseZones == true then
	CreateThread(function()
		for k,v in pairs(Config.Zones.Zones) do
		    local zones = AddBlipForRadius(v.x, v.y, v.z , v.radius) -- Adjust the size of the circle zones here
			SetBlipHighDetail(zones, true)
			SetBlipColour(zones, v.zonecolour)
			SetBlipAlpha (zones, 128)
			local zones = AddBlipForCoord(v.x, v.y, v.z)
