...
for k, v in pairs(Config.JobSettings.Locations.Boss) do
    exports[TargetName]:AddBoxZone(v.name, v.coords, v.length, v.width, {
        ...
        options = {
            ...
            job = {[Config.JobSettings.JobName] = 3,[Config.JobSettings.JobName] = 4}
            },
        },
        distance = v.distance,
    })
...
AddEventHandler('boii-whitewidow:client:OpenStash', function(data)
    ...
    if not data.forceentry then
        ...
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', data.stash, {
            maxweight = 10000000, -- Adjust storage weight here
            slots = 60, -- Adjust storage slots here
        })
...
CreateThread(function()
    ...
    EdibleTable = CreateObject(`v_ret_fh_kitchtable`, 178.39, -250.39, 52.31, false, false, false)
    SetEntityHeading(EdibleTable,333.72)
