...
const VinScratch = (id, index, price) => {
	if (crypto - price - 350 >= 0) {
...
const Contract = (id, index, price) => {
	...
	return (
        ...
        <button onclick={Wahed()} class="accepted-contract-closeButton"></button>
        <div class="accepted-contract-title-flex">
            <h4 class="accepted-contract-title"> Select Type</h4>
        </div>
        
        <p class="accepted-contract-text">If you choose to VIN scratch it will cost an <br> additional 350 QBit to claim
            ownership.</p>
        <div class="accepted-contract-optionButton-flex">
            <button class="accepted-contract-optionButton" onclick={VinScratch(${id},${index},${price})}>VIN SCRATCH</button>
			...
			return (
				...
				`<div class="contract-cards" id="decline-id${element.id}">
					...
					<div class="contract-buttons-flex">
						<button id="start-contract${counter}" onclick={Contract(${element.id},${counter},${element.price})} class="contract-buttons">Start Contract</button>
