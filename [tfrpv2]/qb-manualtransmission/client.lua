...
RegisterCommand("manual", function()
    if vehicle == nil then
        if manualon == false then
            manualon = true
			TriggerEvent('QBCore:Notify', 'Manual Transmission Actived', 'success')
        else
            manualon = false
			TriggerEvent('QBCore:Notify', 'Manual Transmission Disactived', 'error')
        end
    end
end)
    ...
    if vehicle == nil then
        if manualon == false then
            ...
            if realistic == true then
                realistic = false
				TriggerEvent('QBCore:Notify', 'Beginers Driving mode Enabled', 'info')
            else
                realistic = true
				TriggerEvent('QBCore:Notify', 'Realistic Driving Mode Enabled', 'info')
            end
        end
    end
end)
...
if Config.gearhud == 1 then
    Citizen.CreateThread(function()
        while true do
            ...
            AddTextComponentString("~p~Gear: ~w~"..getinfo(selectedgear))
...
elseif Config.gearhud == 2 then  
    Citizen.CreateThread(function()
        while true do
            ...
            AddTextComponentString("~p~Gear: ~w~"..getinfo(selectedgear).." ~r~Km/h: ~w~"..round((GetEntitySpeed(vehicle)*3.6),0).." ~r~RPM: ~w~"..round(GetVehicleCurrentRpm(vehicle),2))
