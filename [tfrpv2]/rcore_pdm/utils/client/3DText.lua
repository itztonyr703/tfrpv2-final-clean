...
Citizen.CreateThread(function()
    while true do
        ...
        for i, self in pairs(nearTextsV2) do
            ...
            local distance = #(coords - self.position)
            if distance <= self.renderDistance and not self.destroyed and jobCheck then
                self.rendering = true
...
Citizen.CreateThread(function()
    while true do
        ...
        for i, self in pairs(nearTextsV2) do
            ...
            if self.rendering and not self.destroyed and self.stopRendering == false and jobCheck then
                if self.isIn then
