...
GetVehicleProperties = function(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        if Config.Framework == QB and Config.TrimPlate then
            plate = Trim(GetVehicleNumberPlateText(vehicle))
        end
        ...
        return {
            ...
            modDashboardColor 	  = GetVehicleDashboardColor(vehicle),
            modInteriorColor     = GetVehicleInteriorColor(vehicle),
...
SetVehicleProperties = function(vehicle, props)
    ...
    if DoesEntityExist(vehicle) then
        ...
        if props.fuelLevel ~= nil then
            ...
            TriggerEvent('fuel:setFuel', vehicle, props.fuelLevel + 0.0)
        end
        ...
        if props.pearlescentColor ~= nil then SetVehicleExtraColours(vehicle, props.pearlescentColor, wheelColor) end
        if props.wheelColor ~= nil then SetVehicleExtraColours(vehicle, props.pearlescentColor or pearlescentColor, props.wheelColor) end
        ...
        if props.modDashboardColor ~= nil then SetVehicleDashboardColor(vehicle, props.modDashboardColor) end
        if props.modInteriorColor ~= nil then SetVehicleInteriorColor(vehicle, props.modInteriorColor) end
