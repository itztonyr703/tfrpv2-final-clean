...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for i, self in pairs(nearMarkersV2) do
            ...
            if self.getRenderJob() ~= nil then
                jobCheck = IsAtJob(self.getRenderJob())
            end

            if (self.onlyVehicle == nil or IsPedAnyVehicle(getPed(), false) == self.onlyVehicle) and jobCheck == true  then
                local distance = #(coords - self.position)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for i, self in pairs(nearMarkersV2) do
            ...
            if (self.onlyVehicle == nil or IsPedAnyVehicle(getPed(), false) == self.onlyVehicle) and self.stopRendering == false and jobCheck then
                if not self.stopRendering and self.rendering and not self.destroyed then
                    if self.isIn and not self.IsMarkerBusy() then
...
function createMarker(res)
    ...
    ------------------------------
    self.setRenderJob = function(job)
        self.jobRendered = job
    end
    ------------------------------
    self.getRenderJob = function()
        return self.jobRendered
    end
    ------------------------------
    self.setOnlyVehicle = function(result)
        self.onlyVehicle = result
    end
    ------------------------------
    self.setJob = function(str)
        self.JobName = str
    end
    ...
    self.SetBusyStatus = function(value)
        self.isBusy = value
        self.update()
    end

    self.IsMarkerBusy = function()
        return self.isBusy
    end
