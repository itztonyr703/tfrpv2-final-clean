...
AddEventHandler('re2-vault:createStorage', function(location)
    ...
    if cpdialog ~= nil then
        ...
        if result then
            ...
            QBCore.Functions.Notify("Duplicate Name For storage", "error")
        end
        -- return cb(result)
    else
        QBCore.Functions.Notify("You can not Afford that", "error")
    end
    end
...
Citizen.CreateThread(function()
    local blip = AddBlipForCoord(Config.Warehouse) -- Mudar coordenadas do blip aqui!
	SetBlipSprite(blip, 474) -- Mudar estilo do blip aqui!
	SetBlipDisplay(blip, 4)
	SetBlipScale(blip, 0.7)
	SetBlipAsShortRange(blip, true)
	SetBlipColour(blip, 50)
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentSubstringPlayerName("Warehouse") -- Mudar nome do Blip aqui!
