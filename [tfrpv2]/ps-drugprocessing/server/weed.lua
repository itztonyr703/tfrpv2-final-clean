...
RegisterServerEvent('ps-drugprocessing:rollJoint', function()
	...
	if Player.Functions.RemoveItem('weed_skunk', 1) then
		if Player.Functions.RemoveItem('rolling_paper', 1) then
			if Player.Functions.AddItem('weed_skunk_joint', 1) then
				...
				TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['weed_skunk_joint'], "add")
			...
			Player.Functions.AddItem('weed_skunk_joint', 1)
		end
	else
