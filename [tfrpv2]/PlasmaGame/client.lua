...
--end
--function headsUp1(text,x,y,z)
--	local coords = {}
--	coords.x = x
--	coords.y = y
--	coords.z = z+0.5
--	DrawText3Ds(coords,text,0.35)
...
--			if k == "Patochemap5" then
--				for k1,v1 in pairs(v) do
--					if type(v1) == "table" then
--						if v1.x ~= nil then
--							DrawMarker(0, v1.x, v1.y, v1.z , 0, 0, 0, 0, 0, 0, 0.15001, 0.15001, 0.25001, 255, 0, 0,165, 0, 0, 0,0)
--							headsUp1(tostring(k).." "..tostring(k1),v1.x, v1.y, v1.z)
--						end
--					end
--				end
--			end
...
function teleport(coords)
	...
	SetEntityCoords(PlayerPedId(),coords.x+xrand,coords.y+yrand,coords.z+0.1)
end
				...
				if( pdist < 15.0 )then
					...
					if( pdist < 1.0 )then
					    ...
					    if IsControlJustReleased(1, 51) then
							print("k: "..tostring(k).." k1: "..tostring(k1))
							EnableGui(true,k)
						end
...
function startGame(color,idx,manche)
	...
	--SetResourceKvp
	print("Set kvp : "..tostring(currentSession.shop))
	SetResourceKvp("PlasmaShop",tostring(currentSession.shop))
...
function leaveTheGame(winner,maps)
	...
	if maps ~= nil and maps ~= "none" then
		print("Basic leave no kvp")
		teleport(PaintBallShop[maps]["Out"])
	else
		local maps = GetResourceKvpString("PlasmaShop")
		if maps ~= nil and maps ~= "none" then
		print("Kvp Found")
		teleport(PaintBallShop[maps]["Out"])
		else
		print("No kvp found, no basic leave, default leave")
		...
		if isCurrentlyInGame then
			if currentSession.modes ~= "CTF" then
				if GetEntityHealth(PlayerPedId()) ~= startHealth then
					SetEntityHealth(PlayerPedId(),tonumber(startHealth))
				end
				if HasPedBeenDamagedByWeapon(PlayerPedId(),GetHashKey(GunName),0) then
					...
					if not isCurrentlyOutInGame then
						...
						SetEntityHealth(PlayerPedId(),tonumber(GetEntityHealth(PlayerPedId())+GunDamage))
