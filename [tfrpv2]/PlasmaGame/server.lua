...
function sendToPlayerTheNextManche(idx)
	...
	local cptPlayer = 0
	local currentManche = currentPaintBallSession[idx].manche
	print("sendToPlayerTheNextManche id :"..tostring(idx))
	for k,v in pairs(currentPaintBallSession[idx].EquipA) do
...
function NextManche(idx,winner)
	Citizen.CreateThread(function()
		local locidx = idx
		local lociwinner = winner
		--print("1-NextManche id :"..tostring(idx).." winner: "..tostring(winner))
		for k,v in pairs(currentPaintBallSession[locidx].EquipA) do
			TriggerClientEvent("PaintBall:GoToNextMancheMSG",v,lociwinner)
		end
		
		for k,v in pairs(currentPaintBallSession[locidx].EquipB) do
			TriggerClientEvent("PaintBall:GoToNextMancheMSG",v,lociwinner)
		end
		
		Wait(8000)
		--print("2-NextManche id :"..tostring(idx).." winner: "..tostring(winner))
		if currentPaintBallSession ~= nil then
			if currentPaintBallSession[locidx] ~= nil then
				sendToPlayerTheNextManche(locidx)
			end
		end
	end)
end
