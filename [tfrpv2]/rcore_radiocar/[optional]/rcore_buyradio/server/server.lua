...
AddEventHandler("rcore_buyradio:BuyRadioVehicle", function(plate)
    ...
    if xPlayer.getMoney() > Config.RadioPrice then
        if not exports.rcore_radiocar:HasCarRadio(plate) then
            ...
            TriggerClientEvent("rcore_buyradio:showNotification", source, "Your vehicle has now a radio! Enjoy it!")
        else
            TriggerClientEvent("rcore_buyradio:showNotification", source, "Dont be silly, your vehicle already has a radio. :D")
        end
    else
        TriggerClientEvent("rcore_buyradio:showNotification", source, "I am sorry but you do not have enough money for radio! :/ next time maybe.")
    end
end)